<script lang="ts">
  ...
  const currentSelectionKey = $derived(
    selectionKey(term, selectedCourses, blockedHours),
  );
  ...
  const showNoSchedules = $derived(
    !submitting &&
      hasResultForSelection &&
  ...
  const loadCourses = async (termValue: string) => {
    try {
      ...
      coursesError =
        err instanceof Error ? err.message : $t("courseSelector.failedToLoad");
    } finally {
      loading = false;
    }
  };
    ...
    ([termValue, indexValue]) => {
      ...
      try {
        localStorage.setItem(
          getTermKey(termValue, "activeTab"),
          String(indexValue),
        );
      } catch (err) {
  ...
  const currentSchedule = $derived(
    scheduleData?.schedules?.[activeScheduleIndex] ?? null,
  );
...
<section class="course-selector">
  ...
  {#if !isMobile || mobileTab === "courses"}
    <div class="course-form" role={isMobile ? "tabpanel" : undefined} aria-labelledby={isMobile ? "mobile-tab-courses" : undefined}>
    <!-- Search Field -->
        ...
        <div class="search-results" role="listbox" id="course-search-listbox">
          ...
          {:else}
            {#each searchResults as result, index (`${result.course}|${result.group}`)}
              ...
              <button
                type="button"
                class="search-result {index === searchHighlightIndex
                  ? 'active'
                  : ''}"
                role="option"
          ...
          {#if totalWarningCount > 1}
            ...
            >
              ...
              <svg
                ...
                height="18"
                fill="currentColor"
                style="transform: rotate({showWarnings ? 180 : 0}deg); transition: transform 0.2s"
              >
      ...
      {#if scheduleData && scheduleData.schedules.length > 1}
        <div class="schedule-nav">
          <button
            ...
            onclick={() =>
              (activeScheduleIndex = Math.max(0, activeScheduleIndex - 1))}
