...
type CourseConflictMap = Record<
	string,
	CourseConflictDetail[] | Record<string, CourseConflictDetail[]>
>;
...
export const getFileFromTerm = (term: string): string => {
	...
	const baseParts =
		seasonCandidate && knownSeasons.has(seasonCandidate) ? parts.slice(0, -1) : parts;
...
export const generateValidSchedules = (
	...
	const courseOptions: CourseOptions[] = validCourses
		.map((course, originalIndex) => {
			const options: SectionOption[] = (filteredSections[course] ?? []).map(
				([section, sessions]) => {
	...
	const chosenByOriginalIndex: Array<{
		course: string;
		section: string;
		sessions: SessionData[];
	} | null> = Array.from({ length: validCourses.length }, () => null);
...
export const createCourseBlockedHoursDetailedWarning = (
	course: string,
	blockedHours: string
): WarningInfo =>
...
export const createSpecificCourseConflictWarning = (
	course1: string,
	course2: string
): WarningInfo => createWarning(WarningCodes.TIME_CONFLICT_BETWEEN_COURSES, { course1, course2 });
